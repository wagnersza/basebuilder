...
class Manager(object):
    ...
    def install(self):
        ...
        if os.system("apt-get update && apt-get install -y --force-yes %s" % (' '.join(packages))) != 0:
    ...
    def install_composer(self):
        if os.path.isfile(os.path.join(self.application.get('directory'), 'composer.json')):
            ...
            os.system('cd %s && %s install' % (self.application.get('directory'), composer_phar))
            print ('cd %s && %s install' % (self.application.get('directory'), composer_phar))
